...
def create_run(payload: TrainingRunCreate, db: Session = Depends(get_db)):
    ...
    run = TrainingRun(
        ...
        params_json={
            ...
            "augmentation": {
                "enabled": payload.augment_enabled,
                "hsv_h": payload.aug_hsv_h,
                "hsv_s": payload.aug_hsv_s,
                "hsv_v": payload.aug_hsv_v,
                "degrees": payload.aug_degrees,
                "translate": payload.aug_translate,
                "scale": payload.aug_scale,
                "shear": payload.aug_shear,
                "perspective": payload.aug_perspective,
                "fliplr": payload.aug_flip_lr,
                "flipud": payload.aug_flip_ud,
                "mosaic": payload.aug_mosaic,
                "mixup": payload.aug_mixup,
                "copy_paste": payload.aug_copy_paste,
                "erasing": payload.aug_erasing,
            },
            "note": payload.note,
        },
    )
